...
const Analytics = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const avgCount =
          trendValues.slice(0, -1).reduce((a, b) => a + b, 0) / 13;
        if (todayCount > avgCount * 2 && todayCount > 5) {
          setSpikeAlert({
            count: todayCount,
            increase: Math.round((todayCount / (avgCount || 1)) * 100),
          });
        }
  ...
  const mapPoints = useMemo(() => {
    return grievances.filter(
      (g) => g.location && g.location.lat && g.location.lng,
    );
  }, [grievances]);
  ...
  if (loading)
    return (
      <div className="p-8 flex flex-col items-center justify-center py-40">
        ...
        <p className="text-gray-500 :text-gray-400 font-bold animate-pulse text-xs uppercase tracking-widest">
    ...
    <div className="p-8 :bg-gray-900 min-h-screen transition-colors duration-300">
      <div className="mb-10">
        ...
        <p className="text-gray-500 :text-gray-400 mt-1 font-medium italic">
          Data-driven insights for systemic improvement.
        </p>
      </div>
      ...
      {spikeAlert && (
        <div className="mb-8 bg-red-50 :bg-red-900/20 border-2 border-red-200 :border-red-800/50 p-5 rounded-2xl flex items-center gap-4 animate-bounce">
          ...
          <div>
            <h3 className="text-lg font-black text-red-800 :text-red-200 uppercase tracking-tight">
              Active Spike Detected!
            </h3>
            <p className="text-sm text-red-700 :text-red-300 font-medium">
              Today's volume is{" "}
              <span className="underline font-black">
                {spikeAlert.increase}% higher
              </span>{" "}
              than average ({spikeAlert.count} new reports).
            </p>
          </div>
        </div>
      )}
...
const scoreToColor = (s) =>
  s >= 70 ? "#ef4444" : s >= 40 ? "#f97316" : "#10b981";
